...
2. **gate ヒステリシス**（`audio_level.py` の `_callback` 内状態機械）: 開くには `gate_activation_sec`（既定 0.7s）の連続音、閉じるには `gate_release_sec`（既定 0.3s）の連続無音。断続ノイズは連続条件のリセットで蓄積しない
3. **pre-lock-in rewind**（`oltw_follower.py`）: lock-in 前の gate 開放中の前進は「仮」。`unfreeze()` 時に `_pre_lockin_resume_pos` をスナップショットし、次の pre-lock-in `freeze()` で前進していたら `_reseed_at(snapshot)` で巻き戻す（conf streak もクリア、`_post_seek_catchup_pending` を arm）。残響込み 0.7s 超の咳や拍手で gate が開いてもラチェットしない。**lock-in（自動 or 強制）= point of no return** で以後は巻き戻さない。`seek()` は snapshot をクリアする（手動 seek を gate close が黙って巻き戻さないため）
...
- **無音測定ボタン**: `AudioLevelMonitor` を再利用して暗騒音を測定し、`compute_silence_threshold()` (`launch_options.py`) が `median + max(median − p10, MIN_SPREAD_DB=6dB) + 3dB` で閾値を算出する。スプレッド床 6dB は必須: 測定窓が短いため暗騒音が非常に安定して見える（median ≈ p10）ことがあり、床なしだと閾値が median+3dB になって空調サイクルやファン回転数変化程度の揺らぎで gate が開き、**演奏前なのに追随が始まる**（幻想4 マイク運用で実測）。音楽のオンセットは暗騒音より 20dB 以上大きいので 9dB のヘッドルームは検出遅延を生まない。gate は「音量 ≤ 閾値で freeze」なので閾値は暗騒音分布の**上側**に置く必要がある。低いパーセンタイル（下位10%等）をそのまま閾値にすると gate がほぼ発動しなくなり、高いパーセンタイル（95% 等）は測定中の偶発音（咳・会話）で吊り上がる — 下側分布幅の鏡映はその両方を回避する設計。入力ソースの選択にかかわらず常に押せる（測定は常にマイクデバイスから。無効化すると「保存済み config が wav モードだとボタンが押せない」という混乱を生んだ実績があるため、ソース連動の disable は再導入しないこと）
