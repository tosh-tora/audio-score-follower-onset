...
- config 引数なし起動でランチャー GUI が開き、選択内容は選択された config.json の `settings.launcher` ブロック + 既存フラットキー（`mic_device` / `loopback_device` / `cooldown_seconds`）に保存される
- **無音判定閾値 `silence_threshold_db` は config に保存しない**（マイク・会場依存のセッション値）。ランチャーの「無音測定」で測って `LaunchOptions.silence_threshold_db` に載せ、`main()` が constructor 引数でアプリにメモリ渡しする（`AudioScoreFollowerApp(silence_threshold_db=...)` → `self._silence_threshold_db`）。config には読み書きせず、`save_launcher_settings` は既存 config に残った旧キーを `pop` で除去する。CLI モード（config 引数あり）は None 渡しで `launch_options.DEFAULT_SILENCE_THRESHOLD_DB`(-55.0) にフォールバックし、実行時 ↑/↓・±ボタンで調整。`ConfigLoader.get_silence_threshold_db()` は**廃止**（config から読む経路自体を消した）
...
- **測定で得た閾値そのものは config に保存しない**（マイク・会場依存のセッション値。上記「起動ランチャー GUI」参照）。永続化するのは `margin_db`（式のパラメータ）だけ。閾値は毎セッション測り直す前提
