...
- **無音測定ボタン**: `AudioLevelMonitor` を再利用して暗騒音を測定し、`compute_silence_threshold()` (`launch_options.py`) が `median + (median − p10) + 3dB` で閾値を算出する。gate は「音量 ≤ 閾値で freeze」なので閾値は暗騒音分布の**上側**に置く必要がある。低いパーセンタイル（下位10%等）をそのまま閾値にすると gate がほぼ発動しなくなり、高いパーセンタイル（95% 等）は測定中の偶発音（咳・会話）で吊り上がる — 下側分布幅の鏡映はその両方を回避する設計。入力ソースの選択にかかわらず常に押せる（測定は常にマイクデバイスから。無効化すると「保存済み config が wav モードだとボタンが押せない」という混乱を生んだ実績があるため、ソース連動の disable は再導入しないこと）
