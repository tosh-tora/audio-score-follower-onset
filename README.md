...
| `gate_release_sec` | `0.3` | silence gate が**閉じる**（freeze する）のに必要な連続無音時間（秒）。音符間の短い切れ目で freeze/unfreeze が暴れるのを防ぐ。`0` で即時反転。 |
...
| **L** / 「▶ 演奏開始」ボタン | **マイクモードの初回押下 = 演奏開始の指示**。起動・楽章ロード直後の OLTW は完全待機（咳・物音では一切動かない）で、この押下で追随が解禁される。押しズレは自動補正: 早押しなら音が持続するまで待機（ノイズで動いても lock-in 前は自動巻き戻し）、遅押しなら初回探索（`start_search_seconds`、既定 10 秒）で実位置に着地。**2 回目以降の押下**（および wav / loopback モード）は従来の強制 lock-in（慣性 arm）。ボタンは lock-in 成立後に自動非表示。L キーは常時有効 |
